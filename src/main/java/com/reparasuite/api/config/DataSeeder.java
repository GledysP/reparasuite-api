...
public class DataSeeder {
	...
	@Bean
	CommandLineRunner seed(TallerRepo tallerRepo, UsuarioRepo usuarioRepo, ClienteRepo clienteRepo,
			OrdenTrabajoRepo otRepo, PasswordEncoder encoder) {
		return args -> {
			...
			// 2) Usuarios
			crearUsuarioSiNoExiste(usuarioRepo, encoder, "admin", "Administrador", "admin@reparasuite.com", "admin123",
					RolUsuario.ADMIN);
			crearUsuarioSiNoExiste(usuarioRepo, encoder, "tec1", "Técnico Uno", "tec1@reparasuite.com", "tec123",
					RolUsuario.TECNICO);
			crearUsuarioSiNoExiste(usuarioRepo, encoder, "tec2", "Técnico Dos", "tec2@reparasuite.com", "tec123",
					RolUsuario.TECNICO);
	...
	private void crearUsuarioSiNoExiste(UsuarioRepo repo, PasswordEncoder encoder, String user, String nombre,
			String email, String pass, RolUsuario rol) {
		if (repo.findByUsuario(user).isPresent())
			return;
	...
	private Cliente crearClienteSiNoExiste(ClienteRepo repo, String nombre, String tel, String email) {
		...
		return repo.findAll().stream().filter(c -> c.getNombre().equalsIgnoreCase(nombre)).findFirst().orElseGet(() -> {
